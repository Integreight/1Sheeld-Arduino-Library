    ...
    if(data==-1)return;
     if(!framestart&&data==START_OF_FRAME)
          ...
          else if(counter==5&&framestart)                      //data is the no of arguments
          {
              ...
              argumentL=(byte*)malloc(1*argumentnumber);//new byte [argumentnumber];
              #ifdef DEBUG
              Serial.print("M2 ");
              #endif
          ...
          else if (counter==7&&framestart)                    // data is the first argument Data information
          {
            ...
            if((255-argumentL[argumentcounter])==data){
              ...
              {
                arguments[argumentcounter]=(byte*)malloc(1*argumentL[argumentcounter]); // assigning the second dimensional of the pointer
                #ifdef DEBUG
                Serial.print("M3 ");
                #endif
                counter++;
