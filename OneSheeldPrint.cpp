...
{
	OneSheeld.sendPacket(shieldId,0,WRITE_ID+functionsIdOffset,1,new FunctionArg(1,(byte *)&data));
}

void PrintClass::print(char data)
{
	OneSheeld.sendPacket(shieldId,0,PRINT_ID+functionsIdOffset,1,new FunctionArg(1,(byte *)&data));
}
...
void PrintClass::print(int data, byte base)
{
	...
	OneSheeld.sendPacket(shieldId,0,PRINT_ID+functionsIdOffset,1,new FunctionArg(strlen(stringPointer),(byte *)stringPointer));
}
...
{
	OneSheeld.sendPacket(shieldId,0,PRINT_ID+functionsIdOffset,1,new FunctionArg(strlen(stringData),(byte*)stringData));	
}
...
void PrintClass::print(double data , int precesion)
{
	...
	OneSheeld.sendPacket(shieldId,0,PRINT_ID+functionsIdOffset,1,new FunctionArg(strlen(buffer),(byte *) buffer));
}
